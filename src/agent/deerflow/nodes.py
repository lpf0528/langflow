...
from agent.deerflow.node_tools import (
    handoff_after_clarification,
    handoff_to_planner,
    direct_response,
)
from agent.deerflow.utils import (
...
def coordinator_node(
    ...
    if not enable_clarification:
        ...
        current_response = (
            clarification_history[-1] if clarification_history else "No response"
        )
...
def planner_node(state: State, config: RunnableConfig):
    ...
    modified_state["messages"] = [{"role": "user", "content": state["research_topic"]}]
...
def human_feedback_node(state: State, config: RunnableConfig):
    ...
    if not state.get("auto_accepted_plan", False):
        ...
        if user_feedback.startswith("[EDIT_PLAN]"):
            ...
            return Command(
                update={
                    "messages": [HumanMessage(content=user_feedback, name="feedback")]
                },
                goto="planner",
            )
    goto = "research_team"
...
async def _setup_and_execute_agent_step(
    ...
    if configuration.mcp_settings:
        for server_name, server_config in configuration.mcp_settings["servers"].items():
            # 启用工具并当前节点在mcp的配置中
            if (
    ...
    if mcp_server:
        # 用于连接多个 MCP 服务器并加载 LC 工具/资源的客户端。
    ...
    # token_limit：最大令牌限制
    # preserve_prefix_message_count：上下文开头要保留的消息数量
    # 固定一个函数的部分参数，返回一个新的函数。
    pre_model_hook = partial(ContextManager(8000, 3).compress_messages)
