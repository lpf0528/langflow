...
class MCPServerMetadataRequest(BaseModel):
    ...
    transport: str = Field(..., description="MCP服务的链接类型，可选值：stdio、sse、streamable_http")
    ...
    timeout_seconds: Optional[int] = Field(None, ge=1, le=3600, description="超时时间（秒）")
    sse_read_timeout: Optional[int] = Field(None, ge=1, le=3600, description="SSE读取超时时间（秒）")
