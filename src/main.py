...
async def _astream_workflow_generator(
    messages: List[dict], thread_id: str, enable_clarification: bool, mcp_settings: dict
):
    workflow_input = {
        "messages": messages,
    ...
    workflow_config = {"thread_id": thread_id}
    # 流式调用图
...
async def _process_message_chunk(
    message_chunk: BaseMessage, message_metadata: dict[str, Any], thread_id, agent
):
    # 处理消息块
    # TODO 空的
        ...
        if index is not None:
            ...
            if chunk_name:
                ...
                if stored_name and stored_name != chunk_name:
                    logger.warning(
                        f"工具索引 {index} 名称不一致: {stored_name} != {chunk_name}"
                    )
                else:
...
async def _stream_graph_events(
    graph: StateGraph, workflow_input, workflow_config, thread_id
):
